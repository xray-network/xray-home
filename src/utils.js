...
export const format = (x, precision = 0) => {
  return precision
    ? parseInt(x)
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
        (parseFloat(x) - parseInt(x)).toFixed(precision).toString().replace("0.", ".").replace("1.", ".")
    : parseInt(x)
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",")
}
...
export const processAsset = (asset) => {
  if (asset) {
    ...
    const metadataNft = process721Metadata(metadataRaw, asset.policyId, assetName)
    const minterr = metadataNft ? metadataNft.publisher === "https://minterr.io" : false
    let quantity = 0
